...
void main()
{
    ...
    col+=smoothcolor*vec3(.9922 * (0.05+cos(u_time * 1.5) / 1.5),.1843 * (.05+cos((u_time + 0.1) * 2.) / 1.5),.9745 * (.05 + sin((u_time + 2.) * 2.) / 1.5))*10.  ;
    float r=.4+.4*(sin(smoothcolor*.1+u_time*.63));
    float g=r*r*.7;
    float b=r*g*10.;
    
    gl_FragColor=vec4(.1-(col+vec3(0.149, 0.0667, 0.3412)),1.);
